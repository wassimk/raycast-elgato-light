...
import { getIPAddresses, dayLightBrightness, dayLightTemperature, openPreferences } from "./lib/utils";
...
export default async function Main() {
  ...
  if (ipAddresses.length === 0) {
    ...
    for (const ipAddress of ipAddresses) {
      try {
        console.log(
          `on --ip-address ${ipAddress} --temperature ${dayLightTemperature()} --brightness ${dayLightBrightness()}`,
        );
        await execute(
          `on --ip-address ${ipAddress} --temperature ${dayLightTemperature()} --brightness ${dayLightBrightness()}`,
        );
